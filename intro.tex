...
ages in particular (because the bar is so low to begin with).
However, it is not the method that will revolutionise stellar populations
because it is still a `boutique' method, applied to hand-picked, bright,
text-book Solar-like oscillators.
In order to produce a catalogue of stellar ages large enough to be useful for
studies of stellar populations, we need a bulk method.
A paint-brush large enough to paint a wall rather than decorate a tea cup.
